...
import { IconFlow } from '@/components/icons/flow';
...
const NewFlowCard = () => {
    ...
    useEffect(() => {
        const savedMessage = localStorage.getItem('savedNewFlowCard');
        if (savedMessage) {
    ...
    const saveMessage = () => {
        ...
        try {
            ...
            localStorage.setItem('savedNewFlowCard', JSON.stringify({ content: updatedBlocks }));
    ...
    return (
        <Card className='w-[420px] backdrop-blur-sm bg-white/5'>
            <CardHeader className='bg-[#c700c7] rounded-t-xl p-3'>
                <Handle
                    type='source'
            ...
            <CardContent className='flex-shrink-0'>
                ...
                <Flex className='text-center mt-6' justify='center' gap='2'>
                    <Button onClick={() => setShowModal(true)} className='justify-center bg-white'>Nueva función del flujo <Plus /></Button>
                </Flex>
            </CardContent>
